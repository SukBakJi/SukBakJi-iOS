...
class UpComingCalendarCollectionViewCell: UICollectionViewCell {
    ...
    func prepare(upComingResult: UpComingResult) {
        dDayLabel.text = String(upComingResult.dday)
        if upComingResult.univId == 1 {
            univLabel.text = "서울대학교"
        } else if upComingResult.univId == 2 {
            univLabel.text = "연세대학교"
        } else if upComingResult.univId == 3 {
            univLabel.text = "고려대학교"
        } else {
            univLabel.text = "카이스트"
        }
        contentLabel.text = upComingResult.content
    }
}
