...
class MyAlarmTableViewCell: UITableViewCell {
    ...
    func prepare(alarmListResult: AlarmListResult) {
        ...
        self.alarmNameLabel.text = alarmListResult.alarmName
        self.alarmDateLabel.text = "\(alarmListResult.alarmDate) \(alarmListResult.alarmTime)"
        if alarmListResult.onoff == 1 {
