...
class SignUpViewController: UIViewController {
    ...
    private func openSelectSignUp() {
        let selectSignUpVC = SelectSignUpViewController()
        selectSignUpVC.modalPresentationStyle = .pageSheet
        
        if let sheet = selectSignUpVC.sheetPresentationController {
            sheet.detents = [.medium()]
            sheet.delegate = self
            sheet.prefersGrabberVisible = true
        }
        present(selectSignUpVC, animated: true, completion: nil)
    }
